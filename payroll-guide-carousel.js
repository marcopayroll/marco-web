...
(function() {
  ...
  function buildCard(c) {
    ...
    var thumbHtml = c.img
      ...
      : '';
    var dateText = c.date
      ? t('pg.card.updated', 'Last Updated by,') + ' ' + t('pg.date.' + countrySlug(c), c.date)
      : 'Content Coming Soon';
    a.innerHTML =
      '<div class="explore-card-thumb">' + thumbHtml + '</div>' +
      '<div class="explore-card-body">' +
        '<div class="explore-card-title-row">' +
          '<span class="explore-card-name">' + t('pg.country.' + countrySlug(c), c.name) + '</span>' +
  ...
  function render(query) {
    ...
    filtered = EXPLORE_COUNTRIES.filter(function(c) {
      ...
      return c.link.replace(/\.html$/, '') !== currentUrl && (!q || normalize(c.name).indexOf(q) !== -1 || normalize(t('pg.country.' + countrySlug(c), c.name)).indexOf(q) !== -1);
    });
    track.innerHTML = '';
